...
LOCAL MMDBW_network_s resolve_network(const char *ipstr, uint8_t mask_length);
LOCAL uint8_t *maybe_add_data_to_table(MMDBW_tree_s *tree, SV *key,
                                       SV *raw_data);
LOCAL void delete_reserved_network(MMDBW_tree_s *tree, char *ipstr,
                                   uint8_t mask_length);
...
MMDBW_tree_s *new_tree(uint8_t ip_version, uint32_t nodes_per_alloc)
{
    MMDBW_tree_s *tree = malloc(sizeof(MMDBW_tree_s));
    if (NULL == tree) {
        abort();
    }
...
int insert_network(MMDBW_tree_s *tree, char *ipstr, uint8_t mask_length,
                   SV *key, SV *data)
{
    MMDBW_network_s network = resolve_network(ipstr, mask_length);

    if (network.gai_status) {
        return network.gai_status;
    }
    ...
    uint8_t *md5_key = maybe_add_data_to_table(tree, key, data);
    MMDBW_record_s new_record = {
...
LOCAL MMDBW_network_s resolve_network(const char *ipstr, uint8_t mask_length)
{
    ...
    network.gai_status = getaddrinfo(ipstr, NULL, &ai_hints, &addresses);
    if (network.gai_status) {
        return network;
    }
                                       ...
                                       SV *raw_data)
{
    MD5_CTX context;
    MD5Init(&context);
    MD5Update(&context, (uint8_t *)SvPVbyte_nolen(key), SvCUR(key));

    uint8_t md5_key[MD5_DIGEST_LENGTH];
    MD5Final(md5_key, &context);

    /* XXX - in the final version the raw data will be an SV instead of plain
     * bytes. */
    MMDBW_data_s *data = NULL;
    HASH_FIND(hh, tree->data_table, &md5_key, MD5_DIGEST_LENGTH, data);

    if (NULL == data) {
        data = malloc(sizeof(MMDBW_data_s));
        data->data = raw_data;
        data->md5_key = malloc(MD5_DIGEST_LENGTH);
        memcpy(data->md5_key, md5_key, MD5_DIGEST_LENGTH);
...
void delete_reserved_networks(MMDBW_tree_s *tree)
{
    if (tree->ip_version == 4) {
        for (int i = 0; i <= 13; i++) {
            delete_reserved_network(tree, ipv4_reserved[i].ipstr,
        ...
        for (int i = 0; i <= 5; i++) {
            delete_reserved_network(tree, ipv6_reserved[i].ipstr,
...
void alias_ipv4_networks(MMDBW_tree_s *tree)
{
    ...
    MMDBW_network_s ipv4_root_network = resolve_network("::0.0.0.0", 96);
    ...
    for (int i = 0; i <= 1; i++) {
        MMDBW_network_s alias_network =
            resolve_network(ipv4_aliases[i].ipstr, ipv4_aliases[i].mask_length);
                                     ...
                                     MMDBW_record_s *new_record)
{
    ...
    if (MMDBW_RECORD_TYPE_DATA == new_record->type) {
        record_to_set->value.md5_key = new_record->value.md5_key;
...
LOCAL MMDBW_node_s *make_next_node(MMDBW_tree_s *tree, MMDBW_record_s *record)
{
    ...
    if (MMDBW_RECORD_TYPE_DATA == record->type) {
        ...
        next_node->right_record.value.md5_key = record->value.md5_key;
        next_node->left_record.type = MMDBW_RECORD_TYPE_DATA;
        next_node->left_record.value.md5_key = record->value.md5_key;
    } else {
        next_node = new_node(tree);
    }
...
char *md5_to_hex(uint8_t digest[16])
{
    char *hex = malloc(33);
    for (int n = 0; n < 16; ++n) {
        snprintf(&(hex[n * 2]), 16 * 2, "%02x", digest[n]);
    }
    return hex;
}

SV *data_for_key(MMDBW_tree_s *tree, uint8_t *md5_key)
{
    MMDBW_data_s *data = NULL;
    HASH_FIND(hh, tree->data_table, md5_key, MD5_DIGEST_LENGTH, data);
    if (NULL != data) {
...
uint64_t node_count(MMDBW_node_s *node)
{
    uint64_t count = 1;

    if (MMDBW_RECORD_TYPE_NODE == node->left_record.type) {
        count += node_count(node->left_record.value.node);
    }

    if (MMDBW_RECORD_TYPE_NODE == node->right_record.type) {
        count += node_count(node->right_record.value.node);
    }
...
char *node_type_name(int node_type)
{
