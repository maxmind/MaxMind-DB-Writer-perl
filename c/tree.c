                                       ...
                                       SV *raw_data)
{
    ...
    if (NULL == data) {
        data = checked_malloc(sizeof(MMDBW_data_s));
        data->data = raw_data;
        data->md5_key = checked_malloc(MD5_DIGEST_LENGTH);
...
/* void write_tree(MMDBW_tree_s *tree, FILE *fd, SV *encoder) */
/* { */
/*     tree->data_encoder = encoder; */

/*     start_iteration(tree, &encode_node); */

/*     tree->data_encoder = NULL; */
/* } */
...
LOCAL void assign_node_number(MMDBW_tree_s *tree, MMDBW_node_s *node)
{
    node->node_number = tree->node_count++;
    return;
}
