...
LOCAL void insert_resolved_network(MMDBW_tree_s *tree, MMDBW_network_s *network,
                                   SV *key_sv, SV *data)
{
    char *key = store_data_in_tree(tree, key_sv, data);
    MMDBW_record_s new_record = {
                                     ...
                                     MMDBW_record_s *new_record)
{
    ...
    if (MMDBW_RECORD_TYPE_DATA == new_record->type
        ...
        char *new_key = new_record->value.key;
        char *other_key = other_record->value.key;
    ...
    if (MMDBW_RECORD_TYPE_DATA == record_to_set->type
        ...
        if (tree->merge_record_collisions) {
            ...
            char *new_key = store_data_in_tree(tree, key_sv, merged);
            SvREFCNT_dec(key_sv);
...
SV *merge_hashes_for_keys(MMDBW_tree_s *tree, char *key_from, char *key_into,
                          MMDBW_network_s *network)
{
...
SV *data_for_key(MMDBW_tree_s *tree, char *key)
{
    MMDBW_data_hash_s *data = NULL;
